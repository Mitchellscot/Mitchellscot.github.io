...
const getBlogs = async (req: NextRequest) => {
    ...
    if (lastId === null || lastPublishDate === null) {
        return NextResponse.json({ error: 'lastId and lastPublishDate are required' }, { status: 500 });
    }
    const result = await fetchSanityData<BlogPreview[]>(getMoreBlogs(lastPublishDate, lastId));
