    ...
    <>
      <div className={styles.statsCol}>
        <div className={titleClasses}>
          ...
          <div className={dateClasses}>{activity.readableDate}</div>
        </div>

        <div>
          <div className={statClasses}>{activity.duration}</div>
          {activity.readableDistance && (
            <div className={statClasses}>{activity.readableDistance}</div>
          )}
          {activity.pace && <div className={statClasses}>{activity.pace}</div>}
        </div>
      </div>
      <div className={iconColStyles}>
        <ActivityIcon activity={activity.activityType} />
        {!effortMissing && <div className={styles.pill}>{activity.effort}</div>}
      </div>
    </>
  );
}
